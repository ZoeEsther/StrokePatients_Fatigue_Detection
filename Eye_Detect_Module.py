...
class EyeDetector:
    ...
    def get_Gaze_Score(self, frame, landmarks):
        ...
        if self.show_processing and (left_eye is not None) and (right_eye is not None):
            left_eye = Utils.resize(left_eye, 1000)
            right_eye = Utils.resize(right_eye, 1000)
    ...
    def get_mouth_Score(self,frame, landmarks):

        self.keypoints = landmarks
        self.frame = frame
        pts = self.keypoints

        mouth_pts = np.zeros(shape=(12, 2))
        i=0
        for n in range(48, 60):
            mouth = pts.part(n)
            mouth_pts[i] = [mouth.x, mouth.y]
            i += 1  # increasing the auxiliary counter

        mouth_opening_score = (LA.norm(mouth_pts[4] - mouth_pts[8]) + LA.norm(
            mouth_pts[2] - mouth_pts[10])) / (2 * LA.norm(mouth_pts[0] - mouth_pts[6]))

        return mouth_opening_score
